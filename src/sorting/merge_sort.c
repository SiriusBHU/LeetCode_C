           ...
           int (*Compare)(const void *, const void *))
{
    ...
    void *vec_tail = base + mi * elem_bytes;

    // compare and add from little to big
    rank i = 0, j = 0, k = lo;
    while (i < len_head && j < len_tail) {
        if (Compare(vec_head + i * elem_bytes, vec_tail + j * elem_bytes)) {
            if (!memcpy(base + k * elem_bytes,
                        vec_head + i * elem_bytes,
                        elem_bytes)) {
                printf("[%s]-[%d] memory copy error!\n", __func__, __LINE__);
                return -1;
            } i++; k++;
